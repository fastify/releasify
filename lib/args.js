...
const booleanOptions = new Set([
  'help', 'major', 'dry-run', 'no-verify', 'silent',
  'gh-release-edit', 'gh-release-draft', 'gh-release-prerelease', 'gh-release-body'
])
...
function normalizeArgs (args) {
  ...
  while (i < args.length) {
    ...
    if (arg.startsWith('--') || arg.startsWith('-')) {
      ...
      const optionName = arg.startsWith('--') ? arg.slice(2) : arg.slice(1)

      // If it already has =, just add it as-is
      if (eqIndex !== -1) {
        normalized.push(arg)
        i++
        continue
      ...
      const isBoolean = booleanOptions.has(optionName) ||
        (optionName.startsWith('no-') && booleanOptions.has(optionName.slice(3)))

      // Look at next arg to see if it's a value (not another option)
      if (i + 1 < args.length) {
        const nextArg = args[i + 1]
        // If next arg is 'true' or 'false' and current is a boolean, combine them
        if (isBoolean && (nextArg === 'true' || nextArg === 'false')) {
...
module.exports.argsNames = {
  string: ['path', 'tag', 'verbose', 'semver', 'remote', 'branch', 'from-commit', 'to-commit', 'npm-access', 'npm-dist-tag', 'npm-otp', 'gh-token', 'arg'],
  array: ['gh-group-by-label'],
  boolean: ['help', 'major', 'dry-run', 'no-verify', 'silent', 'gh-release-edit', 'gh-release-draft', 'gh-release-prerelease', 'gh-release-body']
}
