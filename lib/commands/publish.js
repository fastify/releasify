...
module.exports = async function (args) {
  ...
  try {
    ...
    // if the user set these setting, the validation is more strict
    if (args.remote && args.branch) {
      CLEAN_REPO.tracking = `${args.remote}/${args.branch}`
    } else {
      delete compare.tracking
    }
...
The 'npm publish' has been done! Check your 'git status' and if necessary run
'npm unpublish ${moduleLink}'`
...
The 'npm publish' and 'git push' has been done!\n Consider creating a release on GitHub by yourself
with this message:\n${releasing.message}`
    throw new Error(messageError)
  }
