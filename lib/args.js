...
function normalizeArgs (args) {
  ...
  while (i < args.length) {
    ...
    if (arg.startsWith('--') || arg.startsWith('-')) {
      ...
      const isBoolean = optionName in optionsConfig && optionsConfig[optionName].type === 'boolean'
...
module.exports.argsNames = {
  string: Object.entries(optionsConfig)
    .filter(([, config]) => config.type === 'string')
    .map(([name]) => name),
  array: Object.entries(optionsConfig)
    .filter(([, config]) => config.multiple === true)
    .map(([name]) => name),
  boolean: Object.entries(optionsConfig)
